    ...
    if (cachedVideos) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          // eslint-disable-next-line react-hooks/set-state-in-effect -- sengaja: hydrate dari localStorage (browser-only) setelah mount, tidak bisa lewat lazy initial state karena akan mismatch dengan SSR
          setAllVideos(parsed);
        }
      } catch (e) {
